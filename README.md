...
GitHub Actions 计划在每小时第 05、35 分钟（UTC）尝试执行：恢复上次合格资料 → 用 ETag 只读检查官方页面 → 使用任务结束即失效的内置 `GITHUB_TOKEN` 认证读取 GitHub 公开搜索接口 → 通过 GitLab 单项目公开接口补齐候选许可证 → 生成带 SHA-256 的发布包。GitHub 定时任务是尽力调度，可能延迟或跳过，不能视为严格半小时 SLA；新鲜度监控会量化近 24 小时计划交付率、估算缺失窗口，并在超过 90 分钟没有任何成功运行且无活跃任务时补发一次同一工作流。GitHub / GitLab 全部计划查询必须完成且不能带有不完整结果，才允许替换上一版合格技术目录。
